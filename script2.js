    ...
    fetch('layloichuc.php')
      ...
      .then(data => {
        ...
        data.forEach(msg => {
          ...
          newMessage.innerHTML = `<strong>${msg.ten}</strong>: <p>${msg.loi_chuc}</p>`;
...
form.addEventListener('submit', function(event) {
    ...
    fetch('luuloichuc.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: `ten=${encodeURIComponent(name)}&loi_chuc=${encodeURIComponent(message)}`
    })
    .then(res => res.text())
    .then(data => {
      if (data === 'ok') {
        ...
        loadMessages(); // tải lại danh sách
      } else {
